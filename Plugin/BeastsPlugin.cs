...
public class BeastsPlugin : BaseSettingsPlugin<BeastsSettings>
{
    ...
    private TelemetryUploader _telemetry;
    ...
    public override void OnLoad()
    {
        ...
        // Deferred so they also work before _recorder exists and after an unload.
        // isInFinalizedMap: standing in a map that has already been banked; drives Counter's
        // "complete" readout. hideBeastOverlay: banked AND not standing in it, which also hides
        // world labels, the tracked-beasts window and map markers.
        Func<bool> isInFinalizedMap = () => _recorder?.State.IsInFinalizedMap == true;
        Func<bool> hideBeastOverlay = () =>
        {
            var state = _recorder?.State;
            return state != null && state.MapWasFinalized && !state.IsCurrentAreaTrackable;
        };

        CreateTracking(isInFinalizedMap, hideBeastOverlay);
        CreateAnalytics();
        CreateRenderers(hideBeastOverlay);
        CreateAutomation();
        CreateWorkflows();
        StartServices();
    }
    ...
    private void StartLogging()
    {
        ...
        Log.Attach(
            ...
            () => Settings.LogFile.DebugLogging.Value,
            _logFile);
    ...
    private void CreateTracking(Func<bool> isInFinalizedMap, Func<bool> hideBeastOverlay)
    {
        ...
        _counter = new Counter(GameController, Settings, _tracker, isInFinalizedMap, hideBeastOverlay);
    }
    ...
    private void CreateAnalytics()
    {
        ...
        // A getter, not the current value: _telemetry is assigned below, Render() runs later.
        _analyticsOverlay = new AnalyticsOverlay(GameController, Settings, _recorder, _cost, () => _telemetry);

        // Opt-in telemetry. Enqueue does nothing with sharing off, so subscribing is safe.
        _telemetry = new TelemetryUploader(Settings);
        _recorder.MapFinalized += OnMapFinalizedForTelemetry;
    }

    private void CreateRenderers(Func<bool> hideBeastOverlay)
    {
        _inventoryUi = new InventoryUi(GameController);
        _bestiaryUi = new BestiaryUi(GameController);

        _worldLabels = new WorldLabels(GameController, Settings, _tracker, _prices, hideBeastOverlay);
        _explorationRoute = new ExplorationRoute(GameController, Settings);
        _mapOverlay = new MapOverlay(GameController, Settings, _tracker, _prices, _worldLabels, _explorationRoute, hideBeastOverlay);
    ...
    private void CreateWorkflows()
    {
        ...
        _bestiary = new Bestiary(_runner, _automationInput, _waits, Settings, _bestiaryUi, _inventoryUi, _clipboardAutoPaste, _capturedMonsterStash, _hideoutTravel);
    ...
    // Everything that runs once the objects exist: first price fetch, session housekeeping,
    // settings wiring and the log header.
    ...
    // How this assembly's frames appear in a stack trace, used to tell our faults apart.
    private const string OwnStackMarker = "BeastsV3.";

    // UnobservedTaskException is process-wide: it fires for ExileCore and every other plugin,
    // at an arbitrary later time. Logging all of it at ERROR put other components' failures
    // in our log under our name, so anything without our frames is recorded quietly.
    ...
    // Unsubscribes, saves and disposes, in the order the components depend on each other.
    private void ShutDownServices()
    {
        ...
        if (_recorder != null) _recorder.MapFinalized -= OnMapFinalizedForTelemetry;
        _recorder?.Detach();

        // Last chance to send anything queued. Not awaited - OnClose must not block the host.
        if (_telemetry?.PendingCount > 0) _ = _telemetry.FlushAsync();
    }

    // Drops every component reference. The host keeps drawing the settings object after an
    // unload, so anything still reachable from here would outlive the plugin.
    ...
    public override void AreaChange(AreaInstance area)
    {
        ...
        // An unclassifiable transition counts as a new map. Leaving to hideout or re-entering a
        // finalized map get the soft "demote to cached" reset, so a finalized map keeps showing
        // its frozen totals; only a genuinely new map hash clears counted ids and markers.
    ...
    // Writes the plugin's current state to the log. Reads cached state only, so it is safe mid-run.
    ...
    public override void Render()
    {
        ...
        {
            if (_recorder.State.IsCurrentAreaTrackable) _cost.LatchMapModifiers(now);

            _cost.MaybePoll(now);
            _recorder.Tick(now);
            _webHost.MaybeRefreshSnapshot(now);
            MaybeFlushTelemetry(now);
        }
        ...
        _runner.CheckHotkey(Settings.BestiaryAutomation.YellowItemizeHotkey, "Bestiary yellow itemize",
            () => _bestiary.ItemizeYellowsAsync());
    ...
    public override void DrawSettings()
    {
        _settingsMenu ??= new SettingsMenu(Settings, new MenuContext
        {
            ...
            UpstreamPriceAgeSeconds = () => _prices?.UpstreamAgeSeconds,
        });
    ...
    // Writes the exact bytes that would be uploaded to a file and opens it. Uses real
    // recorded maps, and works whether or not sharing is enabled - reading first is the point.
    private void PreviewTelemetrySubmission()
    {
        try
        {
            var recent = _recorder?.State.MapHistory.Take(5).ToList() ?? [];
            if (recent.Count == 0)
            {
                Log.Info("Telemetry preview: no maps recorded yet. Complete a map first.");
                return;
            }

            var path = _telemetry.WritePreviewFile(recent);
            Log.Info($"Telemetry preview written to {path} ({recent.Count} map(s)). " +
                     $"These bytes would be posted to {TelemetryUploader.EndpointUrl}.");
        ...
        {
            Log.Error("Failed to write the telemetry preview", ex);
        }
    }

    // Under the toggle while sharing is on, so uploads are never a surprise. Plain colored
    // text, not a child window - one would size itself to the panel height, not its content.
    private void DrawTelemetryStatusBanner()
    {
        if (!Settings.Analytics.Telemetry.ShareAnonymousData.Value)
        {
            ImGui.TextDisabled("Off. Nothing is being sent.");
            return;
        }

        var pending = _telemetry?.PendingCount ?? 0;
        var eta = _telemetry?.TimeUntilNextFlush(DateTime.UtcNow);
        var etaNote = eta.HasValue
            ? $"Next upload in {ImGuiEx.FormatDuration(eta.Value)}."
            : "Nothing queued yet - waiting for a map to complete.";
        var bannerNote = Settings.Analytics.Telemetry.ShowActiveBanner.Value
            ? "Also shown as an on-screen banner while Analytics: Overlay is on."
            : "In-game banner is turned off (Show In-Game Banner, above).";

        ImGui.PushTextWrapPos(ImGui.GetCursorPosX() + ImGui.GetContentRegionAvail().X);
        ImGui.TextColored(MenuTheme.Warn,
            $"Anonymous data sharing is ACTIVE - {pending} map(s) queued for upload. {etaNote} {bannerNote}");
        ImGui.PopTextWrapPos();
    }

    // Queues a completed map for upload. A no-op unless sharing is enabled.
    private void OnMapFinalizedForTelemetry(MapAnalyticsRecord record)
    {
        _telemetry?.Enqueue(record);
    }

    // Uploads queued maps periodically rather than per map. Interval-gating lives in
    // TelemetryUploader, so the same countdown backs this call and the UI.
    private void MaybeFlushTelemetry(DateTime nowUtc)
    {
        if (_telemetry == null) return;

        // Fire and forget: an upload must never hold up a render frame. Failures are logged inside.
        _ = _telemetry.MaybeFlushAsync(nowUtc);
    }
    ...
    private void WireDiagnosticsButtons()
    {
    ...
    private void WireTelemetryButtons()
    {
        Settings.Analytics.Telemetry.StatusBanner.DrawDelegate = DrawTelemetryStatusBanner;
        Settings.Analytics.Telemetry.PreviewSubmission.OnPressed = PreviewTelemetrySubmission;
        Settings.Analytics.Telemetry.UploadNow.OnPressed = () =>
        {
            if (_telemetry == null || _telemetry.PendingCount == 0)
            {
                Log.Info("Telemetry: nothing queued to upload.");
                return;
            }

            var count = _telemetry.PendingCount;
            // Fire and forget, same as the periodic flush - must never block a render frame.
            _ = _telemetry.ForceFlushAsync(DateTime.UtcNow);
            Log.Info($"Telemetry: manually uploading {count} map(s).");
        };
        Settings.Analytics.Telemetry.ResetInstallId.OnPressed = () =>
        {
            var id = _telemetry?.ResetInstallId();
            Log.Info($"Telemetry install id reset to {id}. Data already sent keeps the old id.");
        };
    }

    private void WirePriceButtons()
    {
    ...
    private void WireAnalyticsButtons()
    {
        ...
        Settings.Analytics.CompleteCurrentMap.OnPressed = () =>
        {
            var banked = _recorder.CompleteCurrentMapManually(DateTime.UtcNow);
            Log.Info(banked
                ? "Current map completed manually and banked."
                : "Nothing to complete - not in a trackable map, or no progress yet.");
        };
    }

    private void WireRouteButtons()
    {
    ...
    private void WireDashboardButtons()
    {
    ...
    private void WireTimingButtons()
    {
    ...
    // Hands a path or URL to the shell, which is how both the log folder and the dashboard open.
    ...
    // A preset stamps a coherent set of humanization values; every knob stays editable.

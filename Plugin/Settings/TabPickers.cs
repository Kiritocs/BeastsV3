...
// Dropdown pickers for settings that name a stash tab or map. Names are only readable
// while the owning panel is open, so a closed panel shows the current value and a hint.
public sealed class TabPickers
{
    ...
    public void DrawFaustusShopTab()
    {
        ...
        // Numbered and pinned like stash tabs: Faustus hands out several tabs with the same
        // name, and position is the only thing separating them.
    ...
    public void DrawAtlasMap()
    {
        ...
        // Normalized on draw so older stored values still match a combo entry.
    ...
    // Filter text per combo, and which combos were open last frame so the box can take focus
    // as one opens. A full stash is dozens of tabs.
        ...
        string emptyPreview, bool numbered, bool allowClear)
    {
        ...
        // Resolved rather than string-compared, so a pinned value ticks the tab it points at.
        ...
        for (var i = 0; i < names.Count; i++)
        {
            ...
            // The index prefix is label-only; the value stays the tab name, pinned with its position
            // when the name alone cannot identify it.
    ...
    // An add/remove list rather than fixed slots: how many overflow tabs someone needs depends
    // on how long they run unattended, and an empty list is the natural way to say "none".
    // resolveIndex is null for lists matched by name, and non-null for stash tabs, where a
    // position can be pinned to tell apart tabs sharing a name.
        ...
        bool numbered, string emptyHint, Func<string, int> resolveIndex)
    {
        ...
        {
            if (DrawTabSlot(id, tabs, slot, names, numbered, resolveIndex, duplicateNames)) removeAt = slot;
        }
    ...
    // One row of the list: the tab combo, its remove button and the duplicate warning.
    // True when this row's X was clicked.
    private bool DrawTabSlot(string id, List<string> tabs, int slot, IReadOnlyList<string> names,
        bool numbered, Func<string, int> resolveIndex, HashSet<string> duplicateNames)
    {
        var remove = false;

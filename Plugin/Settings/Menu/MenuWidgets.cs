...
public static class MenuWidgets
{
    ...
    private static void DrawHotkey(MenuItem item, HotkeyNodeV2 hotkey, string id)
    {
        ...
        // Popped before any early return, or a cancelling click would leak a color onto the host style.
        ...
        // Two phases, and the split is the point: binding on key-down leaves the key physically
        // held while the workflow is already listening, so the hotkey fires itself as you assign
        // it. Committing on release means there is no press left to see.
        ...
        if (!TrySetHotkey(item, hotkey, target))
            Log.Error($"Could not rebind '{item.Label}' - this ExileCore build stores hotkeys in a shape the menu does not recognize.");
    }
    ...
    private static void BeginCapture(string itemId)
    {
        ...
        // Whatever is already down when capture opens is the Space or Enter that activated the
        // button, not a choice. Those keys become bindable again once released.
    ...
    private static bool TryReadPressedKey(out Keys key)
    {
        foreach (var candidate in BindableKeys)
        {
            ...
            {
                // Held since capture opened, so not a deliberate choice. A later press counts normally.

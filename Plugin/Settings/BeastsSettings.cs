...
public class BeastsSettings : ISettings
{
    ...
    [Menu("Overlays: Map & World", "In-world beast labels, large-map markers, tracked-beasts floating window, and inventory/stash/merchant price panels.")]
    ...
    [Menu("Automation: Merchant (Faustus)", "List itemized captured beasts to Faustus.")]
    ...
    [Menu("Automation: Bestiary Clipboard", "Auto-copy + auto-paste search regex when the Bestiary panel opens.")]
    ...
    [Menu("Automation: Status Overlay", "Automation status message shown at the top of the screen while running.")]
    public AutomationStatusOverlaySettings AutomationStatus { get; set; } = new();

    [Menu("Diagnostics: Verbose Logging", "Emit detailed step-by-step logs to the ExileCore console. Useful when troubleshooting; noisy otherwise. Does not affect the log file, which always records everything.")]
...
public class BeastPricesSettings
{
    ...
    [Menu("Tracked Beasts", "One row per beast with two independent choices. Track counts it as valuable everywhere: overlays, the Bestiary regex, completion and analytics. Talisman only puts it on overlays in the talisman colours so you can spot it - it stays out of the regex, completion and analytics. Tick both to do both.")]
    [JsonIgnore]
...
[Menu("Talisman-Only World Text", "Colour for a beast shown only because its talisman is selected.")]
    ...
    [Menu("Tracked Window Cached Tag", "Colour of the tag marking a beast that is only remembered, not currently loaded. Muted by default so it reads as an annotation rather than a status.")]

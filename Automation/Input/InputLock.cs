...
public sealed class InputLock : IDisposable
{
    ...
    // The hooks are owned by this thread, which pumps messages so the OS can deliver
    // callbacks. See EnsureHooksInstalled for why that matters.
    ...
    // Installs the keyboard and mouse hooks if they are not already active.
    // Installs the hooks on a dedicated thread that pumps messages, and blocks until they
    // are up.
    ...
    private void HookThreadMain()
    {
        ...
        // The message loop. Callbacks are delivered while this pumps; without it the OS
        // times out on every event. GetMessage returns 0 on WM_QUIT, which StopHookThread
        // posts.
        try
        {

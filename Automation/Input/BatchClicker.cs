...
//
// The serial alternative - ctrl-click an item, wait for the game to confirm it moved, repeat -
// spends a full round trip per item, and that round trip is where nearly all of a transfer
// loop's runtime goes. Inventory, stash and map-device grids are fixed: a cell does not
// reflow when its neighbour empties, so every position collected before a pass is still the
// right position after earlier clicks in that pass have landed. That makes one settle per
// pass correct rather than merely faster.
//
// Ctrl is held down for the whole pass instead of being tapped per click, which is both
// quicker and closer to what a person does.
public sealed class BatchClicker
{
    ...
    // Ctrl-clicks every position with Ctrl held throughout, returning how many clicks were
    // sent. `guard` is re-checked before each click: a panel closing part-way through a pass
    // would otherwise send the remaining clicks into the world.
        ...
        Func<bool> guard = null)
    {
        ...
        // A modifier left down by an earlier run would make the first click a shift- or
        // ctrl-combination of its own.
    ...
    // Polls `read` until `isComplete` holds, the timeout expires, or the value has moved and
    // then stayed put for the settle window.
    //
    // The settle window is what keeps a dropped click cheap: without it, a pass that moved
    // 19 of 20 items would sit on the full timeout every time instead of noticing within
    // ~100ms that nothing more is coming and letting the caller retry the remainder.
    ...
    // allowance for each additional item, since they pipeline rather than queue up serially.
    ...
    // Centre of an item's cell, or null when the rect is unreadable.
    ...
    // Cell centres for a run of items, skipping any whose rect cannot be read.

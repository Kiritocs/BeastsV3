...
// Tells apart tabs that share a name. Settings identify a tab by name, so two tabs called
// the same thing both resolve to the first one; appending the position ("Beasts#3") picks
// a specific tab. The suffix is only written when a name is genuinely ambiguous, so plain
// names and values saved before pins existed keep working.
public static class TabPin
{
    ...
    // Three passes, in descending order of confidence: an exact match first, so a tab really
    // called "Loot#2" wins; then the pin, the only thing that separates same-named tabs; then
    // the bare name, so a pin whose tab moved still lands somewhere sensible.

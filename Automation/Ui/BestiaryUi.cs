...
public sealed class BestiaryUi
{
    ...
    // True when the Captured Beasts sub-tab is the one showing, proven by reading its row
    // list. Throws internally in the negative case, so keep it off per-frame paths.
    ...
    // Clickable rows and the total matching count, from one pass over the row list.
    //
    // The itemize loop needs both every batch, and asking for them separately meant walking
    // 850+ rows twice — each row read out of process memory — for data that comes from the
    // same enumeration.
    ...
    //
    // A full scan establishes a candidate band of rows near the viewport; later frames
    // re-read only those rects and rescan when the band stops covering the viewport.
    ...
    // Rows the price overlay should draw this frame, each with its rect. The returned list
    // is reused and valid until the next call.
    ...
    // Returns the row list, cached for OverlayRowListTtl; false when the sub-tab is not
    // showing.
    ...
    // The Bestiary entry in the category bar, picked by screen position since child
    // indices rotate.
    public Element TryGetBestiaryCategoryButton()
    {
        ...
        for (var i = 0; i < entriesRoot.ChildCount; i++)
        {
            ...
            // Skips the full-width bar background.
            if (rootWidth > 0 && rect.Width >= rootWidth - 1f) continue;

            entries.Add(entry);
        }

        if (entries.Count <= BestiaryCategoryIndexFromLeft) return null;

        entries.Sort((a, b) => a.GetClientRect().Left.CompareTo(b.GetClientRect().Left));
        return entries[BestiaryCategoryIndexFromLeft];
    }
    ...
    // The Captured Beasts sub-tab: the bottom-most button in the strip, found by position.
    ...
    public Element TryGetCapturedBeastsButtonToClick()
    {
        ...
        for (var i = 0; i < strip.ChildCount; i++)
        {
            ...
            if (rect.Width <= 8 || rect.Height <= 8 || rect.Top <= bottomMost) continue;

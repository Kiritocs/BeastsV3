...
public sealed class StashUi
{
    ...
    // Whether the game reports the cursor over this exact item. Compared on the item entity,
    // since the element wrapper the hover read returns may not be the enumerated instance.
    ...
    public int VisibleTabFreeCellCount() =>
        ServerInventoryGrid.FreeCellCount(Element?.VisibleStash?.ServerInventory, "Stash tab");
    ...
    // Every matching cell in the visible tab, in reading order so batched passes stay followable.

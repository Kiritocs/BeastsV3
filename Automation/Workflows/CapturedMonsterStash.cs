...
public sealed class CapturedMonsterStash
{
    ...
    public async Task<int> StashAllAsync(CancellationToken ct)
    {
        ...
        var normalTabs = ResolveTabChain(_settings.BestiaryAutomation.ItemizedBeastTabs, "Itemized Beasts");
        if (normalTabs.Count == 0)
            throw new InvalidOperationException(
        ...
        var redTabs = ResolveTabChain(_settings.BestiaryAutomation.RedBeastTabs, "Red Beasts");

        // How far along each chain we are. A tab found full is never revisited, so the
        // batching loop below can't ping-pong between a full tab and its successor.
        var normalCursor = 0;
        var redCursor = 0;
        ...
        while (true)
        {
            ...
            // Items are batched per destination tab, one tab switch each. Which tab that is
            // depends on how far each chain has advanced, so it's resolved per iteration.
            var useRed = redTabs.Count > 0 && IsRedBeast(items[0]);
            var targetTabIndex = useRed ? redTabs[redCursor] : normalTabs[normalCursor];

            var batch = items
                .Where(item =>
                {
                    var itemUsesRed = redTabs.Count > 0 && IsRedBeast(item);
                    return itemUsesRed == useRed;
                })
                .ToList();
            ...
            // Beast items are 1x1, so free cells equals how many more fit; -1 means the
            // grid can't be read and the stall detector handles it.
            var freeCells = _stash.VisibleTabFreeCellCount();
            if (freeCells == 0)
            {
                // Full tab: step to the next in this chain and retry rather than failing.
                var chain = useRed ? redTabs : normalTabs;
                var cursor = useRed ? redCursor : normalCursor;
                var label = useRed ? "Red Beasts" : "Itemized Beasts";

                if (cursor + 1 < chain.Count)
                {
                    Log.Info($"Stash tab '{_stash.TabNameAt(chain[cursor])}' is full. " +
                             $"Moving to the next {label} tab '{_stash.TabNameAt(chain[cursor + 1])}'.");

                    if (useRed) redCursor++; else normalCursor++;
                    currentTabIndex = int.MinValue;
                    continue;
                }

                throw new InvalidOperationException(
                    $"Every configured {label} stash tab is full ({string.Join(", ", chain.Select(_stash.TabNameAt))}) - " +
                    $"{items.Count} itemized beast{ImGuiEx.PluralSuffix(items.Count)} still in inventory (moved {moved} so far). " +
                    $"Free up space, or add another tab under Automation: Bestiary -> {label} Stash Tabs.");
            }
    ...
    // Turns configured tab names into stash indices, in order.
    //
    // A name that no longer matches a tab is skipped with a warning rather than failing the
    // run: someone who renamed one tab of three should lose that destination, not the whole
    // auto-stash. An empty result is the caller's business — it means different things for
    // the two chains.
    private List<int> ResolveTabChain(List<string> configured, string label)
    {
        ...
        foreach (var name in configured)
        {
            ...
            // Listing a tab twice would waste an overflow step, since the second visit finds
            // the tab already full.

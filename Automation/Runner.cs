...
public sealed class Runner
{
    ...
    // Checks the panic-stop hotkey. Polled every frame including while idle: the tracker
    // discards the first edge it sees for a key, so idle frames must not be skipped.
    ...
    // The panic stop key must always survive the input lock, or the keyboard hook eats the
    // press before ExileCore can poll it.
    ...
    // Sets the overlay status text and logs it. The host logger collapses identical
    // consecutive lines, so callers should vary repeated messages.

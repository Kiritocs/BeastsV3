...
public sealed class PricePanels
{
    ...
    // Bestiary row names are deliberately NOT cached: the list recycles row elements, so after
    // an itemize the remaining rows keep their addresses and take on different beasts. An
    // address-keyed cache painted the old occupant's name onto the new one.
    ...
    // Exception types already logged, so a repeating failure writes one line, not one per frame.
    ...
    private void DrawBestiaryPanelPrices()
    {
        ...
        {
            // Read fresh every frame so the label matches the row's current beast. An empty read means
            // the tooltip has not populated; skipping draws nothing rather than a stale name.

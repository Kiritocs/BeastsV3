...
public sealed class MapOverlay
{
    ...
    public void Render()
    {
        ...
        if (_settings.MapRender.ShowBeastsOnMap.Value && !_hideBeastOverlay() &&
    ...
    private bool ShouldDrawOverlay() =>
        (_settings.MapRender.ShowBeastsOnMap.Value && !_hideBeastOverlay()) ||
    ...
    private void DrawBeastMarkers(Vector2 mapCenter)
    {
        ...
        foreach (var marker in _tracker.Markers)
        {
            ...
            // Talisman-only beasts are included and colored differently.
            ...
            _worldLabels.DrawMapMarker(marker.BeastName, marker.CaptureState, mapCenter + mapDelta);
        }
    }

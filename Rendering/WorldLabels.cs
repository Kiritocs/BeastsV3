...
public sealed class WorldLabels
{
    ...
    private readonly Func<bool> _hideBeastOverlay;
    private readonly Vector2[] _circleBuffer = new Vector2[RenderPrimitives.UnitCirclePoints.Length];

    // hideBeastOverlay: true once the current/last map has been banked and the player is not
    // standing in it - covers reviewing a finalized map and having left it.
        ...
        Func<bool> hideBeastOverlay)
    {
        ...
        _tracker = tracker;
        _prices = prices;
        _hideBeastOverlay = hideBeastOverlay ?? (() => false);
    }
    ...
    public void RenderTrackedBeastsWindow()
    {
        ...
        var style = BuildTrackedRowStyle();
        ...
        if (ImGui.BeginTable("##BeastsV3TrackedTable", 2, tableFlags))
        {
            ...
            // Fixed width from the widest row, so auto-resize converges in one frame instead of oscillating.
            ImGui.TableSetupColumn("Beast", ImGuiTableColumnFlags.WidthFixed,
                MeasureWidestBeastColumn(showEnabledOnly, showCached, style.CapturingText, style.CapturedText, style.CachedTagText));
    ...
    // Colors and suffixes for the tracked-beasts window, resolved once for the whole list
    // rather than per row.
    private readonly record struct TrackedRowStyle(
        Vector4 BeastName, Vector4 TalismanOnly,
        Vector4 Capturing, Vector4 Captured, string CapturingText, string CapturedText,
        Vector4 CachedTag, string CachedTagText);

    private TrackedRowStyle BuildTrackedRowStyle()
    {
        var cachedTag = _settings.MapRender.CachedTagText.Value?.Trim();

        return new TrackedRowStyle(
            ImGuiEx.ToVec4(_settings.MapRender.Colors.TrackedWindowText.Value),
            ImGuiEx.ToVec4(_settings.MapRender.Colors.TrackedWindowTalismanOnlyText.Value),
            ImGuiEx.ToVec4(GetStatusColor(BeastCaptureState.Capturing)),
            ImGuiEx.ToVec4(GetStatusColor(BeastCaptureState.Captured)),
            " " + GetStatusText(BeastCaptureState.Capturing),
            " " + GetStatusText(BeastCaptureState.Captured),
            ImGuiEx.ToVec4(_settings.MapRender.Colors.TrackedWindowCachedTag.Value),
            string.IsNullOrWhiteSpace(cachedTag) ? null : " " + cachedTag);
    }

    // One row: price, beast name, and the capture-state and cached suffixes.
    private void DrawTrackedRow(TrackedBeastMarker marker, TrackedRowStyle style)
    {
        ImGui.TableNextRow();
        ...
        ImGui.TableNextColumn();
        ImGui.TextColored(
            _prices.IsTalismanOnly(marker.BeastName) ? style.TalismanOnly : style.BeastName,
            marker.BeastName);
        ...
        if (marker.CaptureState != BeastCaptureState.None)
        {
            ...
            ImGui.TextColored(captured ? style.Captured : style.Capturing,
                captured ? style.CapturedText : style.CapturingText);
        }

        // Marks rows whose position and status are frozen from when the beast unloaded.
        if (!marker.IsLive && style.CachedTagText != null)
        {
            ImGui.SameLine(0, 0);
            ImGui.TextColored(style.CachedTag, style.CachedTagText);
        }
    }
    ...
    // Draws one beast marker on the large map, on the foreground draw list so it sits on top.
    internal void DrawMapMarker(string beastName, BeastCaptureState captureState, Vector2 pos)
    {
        ...
        RenderPrimitives.DrawCenteredLabel(
            ImGui.GetForegroundDrawList(), primary, secondary, pos,
    ...
    {
        var priceText = _prices.GetDisplayPriceText(beastName);
        var label = _settings.MapRender.ShowNameInsteadOfPrice.Value
            ? beastName
            : _settings.MapRender.ShowPriceInsteadOfName.Value
    ...
    // Circle color by capture state, falling back to the talisman color.
    ...
    private void DrawPreviewMapLabel(BeastCaptureState captureState)
    {
        ...
        var label = _settings.MapRender.ShowNameInsteadOfPrice.Value ? PreviewBeastName
            : _settings.MapRender.ShowPriceInsteadOfName.Value ? "1c"
            : $"{PreviewBeastName} 1c";

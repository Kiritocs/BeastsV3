...
public sealed class PriceService
{
    ...
    // Selected for its talisman only, which gets the amber overlay colours.
    ...
    // The price string overlays show for a beast, with the talisman price appended when
    // combining is on or the beast is talisman-only.
    ...
    // The fetch currently in flight, so concurrent callers join it instead of starting a
    // second one against poe.ninja.
    ...
    //
    // FetchAsync early-returns while a fetch is running, so calling it directly during a
    // background refresh hands back an already-completed task — which would have let
    ...
    //
    // Exists for the paths that price something for real — listing at Faustus — where a
    // stale number is money rather than a cosmetic label. Returns true when the prices in
    // hand are fresh, false when the refresh failed or timed out.
    //
    // Never throws: poe.ninja being slow or down must not abort a sell run. The caller
    // carries on with the prices it already has, and the log says so.
    ...
    private async Task FetchAsync()
    {
        ...
        try
        {
            ...
            SavePersisted();
            Log.Info($"Prices updated ({_settings.BeastPrices.LastUpdated}).");
        }
        catch (Exception ex)
        {
    ...
    private static async Task<Dictionary<string, float>> FetchTypeAsync(
